...
class Ansel_ActionHandler
{
                ...
                foreach ($images as $image) {
                    ...
                    if (!$gallery->hasPermission($GLOBALS['registry']->getAuth(), Horde_Perms::DELETE)) {
                        ...
                        try {
                            ...
                            $notification->push(
                                sprintf(_("There was a problem deleting photos: %s"), $e->getMessage()),
                                'horde.error'
                            );
    ...
    public static function editActions($actionID)
    {
        ...
        $watermark = Util::getFormData('watermark', $GLOBALS['prefs']->getValue('watermark_text'));
        ...
        try {
            ...
            Ansel::getUrlFor('view', ['view' => 'List'], true)->redirect();
            exit;
        }
        switch ($actionID) {
            case 'modify':
        ...
        switch ($actionID) {
            ...
            case 'modify':
                $view = new Ansel_View_GalleryProperties(
                    [
                        'actionID' => $actionID,
                        'url' => new Horde_Url(Util::getFormData('url')),
                        'gallery' => Util::getFormData('gallery')]
                );
                $view->run();
                exit;
            ...
            case 'do_empty':
                ...
                try {
                    ...
                    $notification->push($e->getMessage(), 'horde.error');
                    Ansel::getUrlFor('default_view', [])->redirect();
                    exit;
                }
                switch ($actionID) {
                    case 'do_delete':
