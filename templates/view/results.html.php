...
<form name="gallery" action="<?php use Horde\Util\Util;

echo Horde::url('view.php', true)?>" method="get">
  ...
  <?php if ($this->total): ?>
    <span class="rightFloat">
      ...
      <?php if (!empty($this->owner)): ?>
        <small><?php echo $this->contentTag('a', _("View All Results"), ['href' => Horde::selfUrl()->add(['view' => 'Results']), 'title' => _("View Results from All Users")]) ?></small>
      <?php endif; ?>
    </span>
  <?php endif; ?>
          ...
          <div style="background-color:<?php echo $this->style->background ?>;">
            ...
            <div id="tags"><ul class="horde-tags">
              <?php foreach ($this->rtags as $id => $taginfo): ?>
                ...
                <?php endif ?>
                <li><?php echo $this->contentTag('a', $this->h($taginfo['tag_name']), ['href' => strval($this->taglinks[$id]), 'title' => sprintf(ngettext("%d photo", "%d photos", $taginfo['total']), $taginfo['total'])]) ?></li>
              <?php endforeach ?>
            </ul></div>
          </div>
        <?php endif; ?>
